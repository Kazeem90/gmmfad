...
for (K in kvals){
  ...
  for (q in qvals){
    ...
    for (p in pp){
      ...
      for (i in 1:n_sim){
        if(i %% 10 == 1) cat("Data simulation nsim =",n_sim,", n =", n, ", p =", p,", q =", q, ", K =", K,", omega=",omega,", it =", i, "\n")
        ...
        prc <- proc.time()
        gmfad <- gmm.fad(Y, K=K, q=q, maxiter = 500, nstart= 20, tol = 1e-6, init= seeds[i] )
        ...
        ### emmixmfa
        prc <- proc.time()
        emmix <- emmix_emEM(Y, K=K, q=q, tol = 1e-6, maxiter = 500, nstart= 20, init = seeds[i])#D_type = 'unique', sigma_type='unique', tol = 1e-6, 
...
################################
################################
#  we shouldn't see j running across kvals before this line. 
for (j in 1:length(kvals)){
  timelist.k[[j]]$K =  rep(kvals[j], dim(timelist.k[[j]])[1])
  ARIlist.k[[j]]$K  =  rep(kvals[j], dim(timelist.k[[j]])[1])
  #frobs.mu.k[[j]]$K =  rep(kvals[j], dim(timelist.k[[j]])[1]) # replace n_sim with length(ARIlist1[[j]]$p)
  #frobs.lambda.k[[j]]$K=rep(kvals[j],dim(timelist.k[[j]])[1])
  #frobs.psi.k[[j]]$K   =rep(kvals[j],dim(timelist.k[[j]])[1])
} 
# merge
timedf.k         <- do.call(rbind, timelist.k) 
ARIdf.k          <- do.call(rbind, ARIlist.k) 

#frobs.mu.df.k    <- do.call(rbind, frobs.mu.k)
#frobs.lambda.df.k<- do.call(rbind, frobs.lambda.k)
#frobs.psi.df.k   <- do.call(rbind, frobs.psi.k)
#
...
#write.csv(frobs.mu.df.k, paste0("gmmfa_emmix_all_frobs_mu_nsim_",n_sim,"_n_",n,"_all_pqK_omega_",omega,"_seed_",seedling,".csv"))
#write.csv(frobs.lambda.df.k, paste0("gmmfa_emmix_all_frobs_lambda_nsim_",n_sim,"_n_",n,"_all_pqK_omega_",omega,"_seed_",seedling,".csv"))
#write.csv(frobs.psi.df.k, paste0("gmmfa_emmix_all_frobs_psi_nsim_",n_sim,"_n_",n,"_all_pqK_omega_",omega,"_seed_",seedling,".csv"))
